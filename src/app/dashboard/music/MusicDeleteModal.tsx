...
export default function MusicDeleteModal({ isOpen, title, artist, onClose, onConfirm }: MusicDeleteModalProps) {
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-[#0e1c1c] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-6 text-center space-y-4 mt-2">
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-white mb-1.5">Hapus Lagu?</h3>
                        <p className="text-sm text-gray-400 leading-relaxed">
                            Yakin ingin menghapus lagu <span className="text-gray-200 font-medium">"{title}"</span> oleh <span className="text-gray-200 font-medium">{artist}</span>?<br />
                            Lagu akan dihapus dari semua playlist yang terkait.
                        </p>
                    </div>
                </div>
