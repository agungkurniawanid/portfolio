...
interface DeployedProjectsDeleteModalProps {
  isOpen: boolean
  title: string
  onClose: () => void
...
export default function DeployedProjectsDeleteModal({ isOpen, title, onClose, onConfirm }: DeployedProjectsDeleteModalProps) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#0e1c1c] border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6 text-center space-y-4 mt-2">
          ...
          <div>
            <h3 className="text-lg font-semibold text-white mb-1.5">Hapus Project Deployed?</h3>
            <p className="text-sm text-gray-400 leading-relaxed">
              Apakah Anda yakin ingin menghapus <br/>
              <span className="text-gray-200 font-medium">"{title}"</span>?<br/>
              Tindakan ini juga akan menghapus thumbnail, gallery, dan file APK di storage secara permanen.
            </p>
          </div>
        </div>
